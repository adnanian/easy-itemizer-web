...
class OrganizationInventoryReport(Resource):
    """Resource tied to the Organization model. Used for sending status reports.
    ...
    def post(self):
        """Generates a status report of the organization's inventory and sends it to all members.
        ...
        if org:
            ...
            member_emails.append("support@easyitemizer.com")
            ...
            low_assignments = [
                ...
                for assignment in org.assignments
                if assignment.current_quantity > 0
                and assignment.current_quantity < assignment.enough_threshold
            ]
...
class OrganizationLink(Resource):
    """Resource tied to the Organization model. Used for generating invitation links.
    ...
    def get(self, name):
        """Generates an invitation link for a request form to join an organization.

        Args:
            name (String): the name of the organization.

        Returns:
            Response: the appropriate response depending on the success of the link generation.
        """
...
api.add_resource(
    OrganizationById, "/organizations/<int:id>", endpoint="organization_by_id"
)
